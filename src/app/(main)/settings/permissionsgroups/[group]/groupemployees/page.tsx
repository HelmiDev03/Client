...
const Employees = () => {
  ...
  useEffect(() => {
    ...
    axios.get(process.env.NEXT_PUBLIC_DOMAIN+`/api/permissions/${group}`)
      .then((res: any) => {
        ...
        setfilteredUsers(users.filter((user: any) => !res.data.permissionGroup.users.some((employee: any) => employee._id === user._id)));
  ...
  const ActionCellRenderer = (props: { row: any }) => {
    ...
    return (
      ...
      <div onClick={() => { setPopupAssigngroup(!PopupAssigngroup); setemployeeName(row.fullname); setemployeeId(row.userid) }} className='p-2 rounded-[50%] w-[35px] height-[35px] flex justify-center items-center border border-gray-300 hover:border hover:border-gray-500'>
  ...
  const rows = groupEmployees.map((user: any, index: number) => ({
  ...
  const [PopupAssigngroup, setPopupAssigngroup] = React.useState(false)
  ...
  return (
    <div>
      ...
      <div style={{ boxShadow: "inset 0 0 10px 0 rgba(0, 0, 0, 0.1)" }} className={` ${PopupAddEmployee ? 'block' : 'hidden'}             p-4 z-10 bg-[#FCFCFC] shadow-lg  absolute w-[500px] translate-x-[180px]  translate-y-[-150px] center rounded-[25px] `}>
        ...
        <div className="w-[90vw] max-w-md">
          <div>
            ...
            <div className='mb-3 font-lexend text-body-2 font-normal text-gray-500 text-sm leading-5 tracking-normal text-left'>Select the employees you would like to assign to this PermissionsGroup.</div>
          </div>
                    ...
                    {inputseacrh ? (
                      ...
                      filteredUsers.length > 0 ? (
                        <>
                          ...
                          <tr>
                            <td colSpan={4} className="text-center py-4">Showing {filteredUsers.length > 3 ? 3 : filteredUsers.length} from {filteredUsers.length} users</td>
                          </tr>
                        </>
                      ) : (
                        <tr>
      ...
      <div style={{ boxShadow: "inset 0 0 10px 0 rgba(0, 0, 0, 0.1)" }} className={` ${PopupAssigngroup ? 'block' : 'hidden'}  h-[300px]           p-4 z-10 bg-[#FCFCFC] shadow-lg  absolute w-[500px] translate-x-[180px]  translate-y-[-60px] center rounded-[25px] `}>
        ...
        <div className="w-[90vw] max-w-md">
          <div className='mb-3'>
            <div className='text-[#16151C] font-lexend font-light text-[20px] leading-[30px] '>Assign to different group</div>
            <div className='mb-3 font-lexend text-body-2 font-normal text-gray-500 text-sm leading-5 tracking-normal text-left'>Choose the new permissions group for {employeeName}.</div>
          </div>
          <div className='mb-6 flex flex-row justify-evenly'>
            <div className='translate-x-[-80px] font-lexend text-body-2 font-normal text-gray-500 text-sm leading-5 tracking-normal text-left'>Current group</div>
            <div className=' font-lexend text-body-2 font-normal text-gray-500 text-sm leading-5 tracking-normal text-left'>New group</div>
          </div>
          <div className="mb-[60px] flex flex-row justify-evenly">
            ...
            <div className='translate-x-[40px] flex' > <FaArrowRight className='translate-y-[10px] translate-x-[-10px] text-gray-500' style={{ fontSize: '20px' }} />  <SelectInput8
              ...
              options={permissionGroups.map((group: any) => group.name).filter((name: any) => name !== Name)}
              value={newgroup}
              onChange={(e: any) => { setnewgroup(e.target.value) }}
            /></div>
          </div>
          <div>
            <div className=' bg-white-500 border-[2px] translate-x-[200px] flex justify-center items-center border-[#7152F3] w-[150px] h-[30px] w-[250px] text-white rounded-[10px] p-1  ' >
              <ButtonSubmit fct={AssignNewgroup} spincol='[#7152F3]' timing={200} text={<h3 className='text-[14px] text-[#7152F3]'>Save</h3>} />
      ...
      {/* end popup assign different group*/}
     ...
     { permission.isadministrators && <div className=' absolute right-[3%] top-[23%]   w-[150px] h-[24px] flex justify-center items-center rounded-[10px] p-[20px] bg-[#7152F3]   ' >
      ...
      <p className="font-lexend text-body-2 font-normal text-gray-500 text-sm leading-5 tracking-normal text-left   ">Here you can assign as many employees as you want to this group.</p>
